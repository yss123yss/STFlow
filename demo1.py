...
def oridata_draw(ori_path):
    ...
    stream_handle = streamdatahandle(stream_data,"T","retweeted_x","retweeted_y","x","y")
    
    #利用Stream Handle，对数据进行统计分析，60*60*24表示按照秒来进行统计
    statistical_info = stream_handle.get_statistical_info('2012-10-22','2012-11-10',60*60*24)
    
    #利用Stream Handle，对数据进行时间区间的裁切，从而获取一段时间内的数据
    stream_data_sliced = stream_handle.slice_stream_data('2012-10-22','2012-11-10')
    ...
    stream_handle.get_statistical_info('2012-10-22','2012-11-10',60*60*24)
    ...
    # stream_handle.gen_period_lines_shp(r'F:\Cluster\论文修改\数据\审稿1\原始数据flowmap\sandy\sandy_total_line.shp', '2012-10-22', '2012-11-10', False, [])
    #按照时间间隔生成所有OD点之间的线数据Shapefile,具体参数看代码吧
    stream_handle.gen_serial_lines_shp(r'F:\Cluster\论文修改\数据\审稿1\原始数据flowmap\sandy\lines','2012-10-22', '2012-11-10', 24*60*60, True)



def result_draw(cluster_file_name, legend_name, cluster_info_name, result_shp_line, result_shp_region, result_pdf):
...
#*******************获取结果切片*************************************
    ...
    acc_val = flow_list_sliced4['count'].sum()*0.90
...
#*******************根据切片绘制结果*************************************
    ...
    # sandy图例位置
    visual_handle.set_flow_visual_parameter(r'F:/Cluster/stream-cluster/World/world.shp','',0.15, 0.135, 0.35, 0.015)
    # visual_handle.get_flow_map_shp(flow_list_sliced1, result_shp_line)
    visual_handle.get_flow_map(flow_list_sliced1, legend_name, result_pdf, [])
...
def result_visual(filepath):
    ...
    # print(filepath+'\n', info_name+'\n', result_shp_line+'\n', result_pdf+'\n')
    result_draw(filepath, legend_name, info_name, result_shp_line, result_shp_region, result_pdf)
    print("文件 %s 处理完毕" %(file_name))
...
if __name__ == '__main__':
    all_result_visual(r"F:\Cluster\论文修改\数据\git-stflow\gitc测试数据\sandy\聚类\result\clusters_den_stream")
    # oridata_draw(r"F:\Cluster\论文修改\数据\审稿1\Sandy_Weather\sandy_weather_export_forDEN.csv")